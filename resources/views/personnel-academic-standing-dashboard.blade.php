<x-layout>
    ...
    <div class="py-12 max-w-7xl mx-auto px-6" x-data='academicStandingDashboard(@json($dashboardPayload), @json(route('api.personnel.academic-standing')))' x-init="fetchDashboard()">
        ...
        <section class="bg-white border border-slate-100 rounded-3xl shadow-sm p-5 mb-6">
            <form x-ref="filterForm" method="GET" action="{{ route('personnel.academic-standing') }}" @submit.prevent="fetchDashboard()" class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-4">
                <div>
    ...
    <script>
        function academicStandingDashboard(payload, apiUrl) {
            return {
                apiUrl,
                loading: false,
