<x-layout>
    <x-slot:title>ลืมรหัสผ่าน | BigData สพป.ชพ.1</x-slot>

    <div class="min-h-[80vh] flex items-center justify-center px-6 relative overflow-hidden">
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-orange-50 rounded-full blur-[120px] opacity-60"></div>

        <div class="w-full max-w-[480px] relative z-10">
            <div class="bg-white/80 backdrop-blur-2xl p-10 md:p-14 rounded-[4rem] border border-white shadow-2xl shadow-orange-100/20">
                
                <div class="text-center mb-10">
                    <div class="w-20 h-20 bg-slate-50 text-orange-500 rounded-[2rem] flex items-center justify-center mx-auto mb-6 shadow-inner transition-transform hover:scale-110 duration-500">
                    ...
                    <h2 class="text-3xl font-black text-slate-800 tracking-tighter italic">Forgot <span class="text-orange-500">Password?</span></h2>
                ...
                @if (session('status'))
                    <div class="mb-6 p-4 bg-orange-50 border border-orange-100 text-orange-600 rounded-2xl text-xs font-bold text-center animate-fade-in">
                        ✨ {{ session('status') }}
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('password.email') }}" class="space-y-6">
                    ...
                    <div class="pt-2">
                        <button type="submit" class="w-full bg-slate-900 text-white py-5 rounded-2xl font-bold text-lg hover:bg-orange-600 transition-all shadow-xl shadow-slate-200 active:scale-95">
                            ส่งลิงก์ตั้งรหัสใหม่ 📧
                        </button>
                    </div>

                    <div class="text-center pt-6">
                        <a href="{{ route('login') }}" class="text-sm font-bold text-slate-400 hover:text-orange-600 transition-colors flex items-center justify-center gap-2">
