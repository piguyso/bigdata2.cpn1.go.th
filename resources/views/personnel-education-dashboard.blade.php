<x-layout>
    <x-slot:title>ข้อมูลแยกตามการศึกษา | BigData สพป.ชพ.1</x-slot>

    <div class="py-12 max-w-7xl mx-auto px-6" x-data='personnelEducationDashboard(@json($dashboardPayload), @json(route('api.personnel.education')))' x-init="fetchDashboard()">
        <header class="mb-8 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
            <div>
                <div class="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-wider">
                    ...
                    <i class="fa-solid fa-chevron-right text-[8px]"></i>
                    <span class="text-slate-600">ข้อมูลแยกตามการศึกษา</span>
                </div>
                <h2 class="text-3xl font-extrabold text-slate-800 tracking-tight mt-1.5">ข้อมูลแยกตามการศึกษา</h2>
                <p class="text-slate-500 text-sm mt-1">
                    รายงานจาก local DB ชุด report05 อิงรูปแบบ HRMS ข้อมูลรายโรงเรียนแยกตามวุฒิการศึกษา
                </p>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                ...
                <a href="{{ route('personnel.education', ['year' => $dashboardPayload['selectedYear'] ?? null, 'term' => $dashboardPayload['selectedTerm'] ?? null, 'school_smis' => $dashboardPayload['selectedSchoolSmis'] ?? null]) }}"
        ...
        <section class="bg-white border border-slate-100 rounded-3xl shadow-sm p-5 mb-6">
            <form x-ref="filterForm" method="GET" action="{{ route('personnel.education') }}" @submit.prevent="fetchDashboard()" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                <div>
        ...
        <div x-show="!dashboard.rows.length" class="bg-white border border-rose-100 rounded-3xl p-10 text-center" x-cloak>
            ...
            <h3 class="text-sm font-extrabold text-slate-700">ไม่พบข้อมูล report05 ในฐานข้อมูล local</h3>
        ...
        <div x-show="dashboard.rows.length" class="space-y-6" x-cloak>
            ...
            <section class="grid grid-cols-1 xl:grid-cols-2 gap-4">
                <template x-for="group in dashboard.roleGroups.slice(0, 2)" :key="group.key">
                    <div class="bg-white border border-slate-100 rounded-3xl shadow-sm p-5">
                        ...
                        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
                            <template x-for="column in summaryColumns" :key="group.key + '-card-' + column.key">
            ...
            <section class="bg-white border border-slate-100 rounded-3xl shadow-sm p-6">
                <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-3 mb-5">
                    <div>
                        <h3 class="font-extrabold text-sm text-slate-800 flex items-center gap-2">
                            <i class="fa-solid fa-table-list text-orange-500"></i> ตารางข้อมูลแยกตามการศึกษารายโรงเรียน
                        </h3>
                ...
                <div class="overflow-x-auto border border-slate-100 rounded-2xl">
                    <table class="min-w-[2300px] w-full text-left text-xs">
                        <thead class="bg-slate-50 text-[10px] uppercase text-slate-500">
                            <tr>
                                ...
                                <template x-for="group in dashboard.roleGroups" :key="'head-group-' + group.key">
                                    <th colspan="7" class="px-4 py-3 font-extrabold text-center border-l border-slate-200" x-text="group.label"></th>
                                </template>
                            </tr>
                            <tr>
                                <template x-for="group in dashboard.roleGroups" :key="'head-' + group.key">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <template x-for="row in dashboard.rows" :key="row.schoolSmis || row.schoolCode || row.index">
                                <tr class="hover:bg-orange-50/40 transition">
                                    ...
                                    <template x-for="group in dashboard.roleGroups" :key="'row-' + row.index + group.key">
                            ...
                            <template x-if="dashboard.rows.length === 0">
                                <tr>
                                    <td colspan="31" class="px-4 py-10 text-center text-slate-400 font-bold">ไม่พบข้อมูลโรงเรียนที่เลือก</td>
                                </tr>
                            </template>
                            <tr class="bg-slate-900 text-white">
                                <td class="px-4 py-4 text-center font-extrabold" colspan="3">รวมทั้งหมด</td>
                                <template x-for="group in dashboard.roleGroups" :key="'total-' + group.key">
    ...
    <script>
        function personnelEducationDashboard(payload, apiUrl) {
            return {
                apiUrl,
                loading: false,
                dashboard: payload || { rows: [], total: {}, overview: [], roleGroups: [] },
                ...
                metricColumns: [
                    { key: 'total', label: 'รวม', className: 'text-slate-800' },
                    { key: 'low', label: 'ต่ำกว่า ป.ตรี', className: 'text-slate-600' },
                    { key: 'edu03', label: 'ป.ตรี', className: 'text-emerald-700' },
                    { key: 'edu035', label: 'ป.บัณฑิต', className: 'text-sky-700' },
                    { key: 'edu04', label: 'ป.โท', className: 'text-orange-700' },
                    { key: 'edu05', label: 'ป.เอก', className: 'text-violet-700' },
                    { key: 'high', label: 'ป.ตรีขึ้นไป', className: 'text-rose-700' },
                ],
                formatNumber(value) {
