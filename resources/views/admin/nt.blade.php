<x-layout>
    <x-slot:title>นำเข้าข้อมูล NT | BigData สพป.ชพ.1</x-slot>

    <div class="py-10 max-w-7xl mx-auto px-6" x-data="ntManager()" x-init="init()">
        ...
        <header class="mb-8 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
            <div>
                <p class="text-xs font-extrabold text-orange-600 uppercase tracking-wider">NT Import</p>
                <h2 class="text-3xl font-extrabold text-slate-900 tracking-tight mt-1">นำเข้าข้อมูล NT</h2>
                <p class="text-slate-500 text-sm mt-1">อัปโหลดไฟล์ CSV หรือ XLSX ผลการประเมิน NT ตรวจสอบก่อนบันทึกจริง และจับคู่โรงเรียนจากชื่อโรงเรียน</p>
            </div>
            <a href="{{ route('admin.nt.template') }}" class="bg-white border border-slate-200 text-slate-650 px-5 py-2.5 rounded-xl font-bold text-xs hover:bg-slate-50 transition shadow-sm inline-flex items-center justify-center gap-2">
                ...
                <div class="p-6 space-y-5">
                    ...
                    <label class="block border-2 border-dashed border-slate-200 rounded-2xl p-5 bg-slate-50/50 hover:border-orange-300 transition cursor-pointer">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-extrabold text-slate-800 truncate" x-text="form.fileName || 'เลือกไฟล์ NT .csv/.xlsx'"></p>
                                <p class="text-xs text-slate-400 mt-1">รองรับไฟล์ข้อมูลดิบ NT 13 คอลัมน์ เช่น NT2568.csv หรือ .xlsx</p>
                            </div>
                    ...
                    <div class="bg-slate-50 border border-slate-100 rounded-2xl px-4 py-3 text-xs flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                        ...
                        <div class="text-slate-400 font-bold" x-show="selectedDataSetSummary()" x-cloak>
                            ...
                            <span x-text="'เฉลี่ยรวม ' + formatScore(selectedDataSetSummary().avg_total_percent)"></span>
                        </div>
                    </div>
                            ...
                            <div class="overflow-x-auto">
                                <table class="w-full text-left text-xs">
                                    <thead class="bg-white text-slate-400 font-bold">
                                        <tr>
                                            <th class="px-4 py-3">รหัส NT</th>
                                            <th class="px-4 py-3">โรงเรียน</th>
                                            <th class="px-4 py-3 text-right">คณิต</th>
                                            <th class="px-4 py-3 text-right">ไทย</th>
                                            <th class="px-4 py-3 text-right">รวม</th>
                                            <th class="px-4 py-3">คุณภาพ</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-slate-50">
                                        <template x-for="row in preview.summary.sample_rows" :key="row.nt_school_code">
                                            <tr>
                                                <td class="px-4 py-3 font-bold text-slate-700" x-text="row.nt_school_code"></td>
                                                <td class="px-4 py-3">
                                                ...
                                                <td class="px-4 py-3 text-right text-slate-500" x-text="formatScore(row.math_percent)"></td>
                                                <td class="px-4 py-3 text-right text-slate-500" x-text="formatScore(row.thai_percent)"></td>
                                                <td class="px-4 py-3 text-right font-extrabold text-slate-700" x-text="formatScore(row.total_percent)"></td>
                                                <td class="px-4 py-3 text-slate-500" x-text="row.total_quality || '-'"></td>
                                            </tr>
                                        </template>
                                    </tbody>
                                </table>
        ...
        <div x-show="deleteModal.open" class="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4" x-transition x-cloak>
            <div class="bg-white rounded-3xl max-w-sm w-full shadow-2xl border border-slate-100 p-6 text-center">
                ...
                <h3 class="font-extrabold text-slate-800 text-sm">ยืนยันการลบข้อมูล NT</h3>
                <p class="text-xs text-slate-400 leading-relaxed mt-2 mb-6">
                    ต้องการลบข้อมูล NT ปีการศึกษา
                ...
                return {
                    ...
                    fetchData() {
                        axios.get('{{ route('admin.nt.data') }}')
                            ...
                            .catch(() => this.showToast('ไม่สามารถโหลดข้อมูล NT ได้', 'error'));
                    },
                    ...
                    previewFile() {
                        ...
                        if (!this.form.file) return this.showToast('กรุณาเลือกไฟล์ NT ก่อน', 'error');
                        this.previewLoading = true;
                        const formData = new FormData();
                        formData.append('academic_year', this.form.academic_year);
                        formData.append('csv', this.form.file);
                        axios.post('{{ route('admin.nt.preview') }}', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
                            .then(response => {
                                this.preview = {
                    ...
                    importFile() {
                        ...
                        axios.post('{{ route('admin.nt.import') }}', {
                    ...
                    deleteDataSet() {
                        ...
                        axios.delete('{{ route('admin.nt.delete') }}', { data: { academic_year: this.deleteModal.academicYear } })
                            .then(response => {
