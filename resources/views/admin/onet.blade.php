                        ...
                        <div class="p-5 space-y-3">
                            <div class="flex items-start justify-between gap-3">
                                <div class="pr-2">
                                    ...
                                    <p class="text-[11px] text-slate-400 mt-1" x-text="'โรงเรียน ' + formatNumber(item.schools_count) + ' • ระเบียน ' + formatNumber(item.records_count)"></p>
                                </div>
            ...
            function onetImportManager() {
                return {
                    ...
                    confirmDeleteImport(item) {
                        window.showConfirm({
                            title: 'ยืนยันการลบข้อมูลนำเข้า ONET',
                            text: `คุณต้องการลบข้อมูลนำเข้า ONET ปีการศึกษา ${item.academic_year} ใช่หรือไม่? ข้อมูลผลคะแนน ONET ทั้งหมดของปีนี้จะถูกลบออกจากระบบโดยสมบูรณ์`,
                            confirmButtonText: 'ลบข้อมูล',
                            cancelButtonText: 'ยกเลิก',
                            type: 'danger',
                            onConfirm: () => {
                                axios.delete('{{ route('admin.onet.delete') }}', { data: { academic_year: item.academic_year } })
                                    .then(response => {
