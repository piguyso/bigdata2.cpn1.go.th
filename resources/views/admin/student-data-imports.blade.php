<x-layout>
    <x-slot:title>นำเข้าข้อมูลนักเรียนเพิ่มเติม | BigData สพป.ชพ.1</x-slot>

    <div class="py-10 max-w-7xl mx-auto px-6" x-data="studentDataImportManager()" x-init="init()">
        ...
        <header class="mb-8 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
            <div>
                <p class="text-xs font-extrabold text-orange-600 uppercase tracking-wider">Student Extra Import</p>
                <h2 class="text-3xl font-extrabold text-slate-900 tracking-tight mt-1">นำเข้าข้อมูลนักเรียนเพิ่มเติม</h2>
                <p class="text-slate-500 text-sm mt-1">นำเข้าไฟล์ CSV หรือ XLSX จาก BIGDATA โดยตรวจสอบโครงสร้างก่อนบันทึกจริง</p>
            </div>
            <div class="flex flex-col sm:flex-row gap-3">
                ...
                <a :href="templateUrl()" class="bg-emerald-600 border border-emerald-600 text-white px-5 py-2.5 rounded-xl font-bold text-xs hover:bg-emerald-700 transition shadow-sm inline-flex items-center justify-center gap-2">
        ...
        <section class="grid grid-cols-1 md:grid-cols-4 gap-3 mb-6">
            ...
            <div class="bg-white border border-slate-100 rounded-2xl px-5 py-4 shadow-sm">
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">ชนิดข้อมูล</p>
                <p class="text-2xl font-extrabold text-slate-900 mt-1" x-text="Object.keys(dataTypes).length"></p>
            </div>
            <div class="bg-white border border-slate-100 rounded-2xl px-5 py-4 shadow-sm">
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">ชุดข้อมูล</p>
                <p class="text-2xl font-extrabold text-slate-900 mt-1" x-text="formatNumber(dataSets.length)"></p>
            </div>
            <div class="bg-white border border-slate-100 rounded-2xl px-5 py-4 shadow-sm">
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">ประวัติล่าสุด</p>
                <p class="text-sm font-extrabold text-slate-900 mt-2" x-text="imports[0] ? 'ปี ' + imports[0].academic_year + ' รอบ ' + imports[0].term : 'ยังไม่มีข้อมูล'"></p>
            </div>
        </section>
            ...
            <section class="bg-white border border-slate-100 rounded-3xl shadow-sm overflow-hidden">
                ...
                <div class="p-6 space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <label class="space-y-1.5 block">
                            ...
                            <select x-model="form.academic_year" class="form-input">
                                ...
                                <template x-for="year in years" :key="year.id || year.year">
                                    <option :value="year.year" x-text="year.name ? year.year + ' - ' + year.name : year.year"></option>
                                </template>
                            </select>
                        </label>
                        <label class="space-y-1.5 block">
                        ...
                        <label class="space-y-1.5 block">
                            ...
                            <select x-model="form.mode" class="form-input">
                                <option value="replace">แทนที่ข้อมูลปี/รอบ/ชนิดนี้</option>
                            </select>
                        </label>
                    </div>
                    ...
                    <label class="space-y-1.5 block">
                        ...
                        <select x-model="form.data_type" class="form-input" @change="resetPreview()">
                    ...
                    <label class="block border-2 border-dashed border-slate-200 rounded-2xl p-5 bg-slate-50/50 hover:border-orange-300 transition cursor-pointer">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-extrabold text-slate-800 truncate" x-text="form.fileName || 'เลือกไฟล์ข้อมูลนักเรียนเพิ่มเติม .csv/.xlsx'"></p>
                                <p class="text-xs text-slate-400 mt-1">ระบบจะตรวจจำนวนคอลัมน์ ปี/รอบ SMIS และการจับคู่โรงเรียนก่อนนำเข้า</p>
                            </div>
                    ...
                    <div class="bg-slate-50 border border-slate-100 rounded-2xl px-4 py-3 text-xs flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                        <div class="text-slate-500">
                            ...
                            <span x-text="selectedDataSetSummary() ? 'มีข้อมูลพร้อมแทนที่' : 'ยังไม่มีข้อมูลที่นำเข้า'"></span>
                        </div>
                        <div class="text-slate-400 font-bold" x-show="selectedDataSetSummary()" x-cloak>
                            <span x-text="'รายการ ' + formatNumber(selectedDataSetSummary().records_count)"></span>
                            <span class="mx-1.5">•</span>
                            <span x-text="'รวม ' + formatNumber(selectedDataSetSummary().total_count)"></span>
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
                            <template x-for="item in summaryCards()" :key="item.label">
                                <div class="bg-slate-50 border border-slate-100 rounded-2xl px-4 py-3">
                                    ...
                                    <p class="text-xl font-extrabold text-slate-900 mt-1" x-text="formatNumber(item.value)"></p>
                                </div>
                            </template>
                        </div>
                                ...
                                <div class="overflow-x-auto">
                                    <table class="w-full text-left text-xs">
                                        ...
                                        <tbody class="divide-y divide-slate-50">
                                            <template x-for="row in preview.summary.sample_rows" :key="row.school_smis + '-' + row.category">
                                                <tr>
                                                    <td class="px-4 py-3 text-slate-500" x-text="row.raw_year_term || '-'"></td>
                                                    ...
                                                    <td class="px-4 py-3 text-right text-slate-700 font-bold" x-text="formatNumber(row.total)"></td>
                                                </tr>
                                            </template>
                                        </tbody>
                                    </table>
                            ...
                            <div class="border border-slate-100 rounded-2xl overflow-hidden">
                                ...
                                <div class="p-4 space-y-3 text-xs">
                                    <div class="flex items-start justify-between gap-3">
                                        <span class="text-slate-400 font-bold">ชนิดข้อมูล</span>
                                        <span class="text-slate-700 font-bold text-right" x-text="selectedTypeLabel()"></span>
                                    </div>
                                    <div class="flex items-start justify-between gap-3">
                                    ...
                                    <div class="flex items-start justify-between gap-3">
                                        <span class="text-slate-400 font-bold">ปี/รอบที่เลือก</span>
                                        <span class="text-slate-700 font-bold text-right" x-text="form.academic_year + '-' + form.term"></span>
                                    </div>
                                    <div class="flex items-start justify-between gap-3">
                                        <span class="text-slate-400 font-bold">โหมดนำเข้า</span>
                                        <span class="text-slate-700 font-bold text-right" x-text="form.mode === 'replace' ? 'แทนที่ข้อมูลปี/รอบ/ชนิดนี้' : form.mode"></span>
                                    </div>
                                    <div x-show="preview.summary?.invalid_samples?.length" class="pt-2 border-t border-slate-100">
                                        <p class="text-slate-400 font-bold mb-2">ตัวอย่างแถวที่ไม่ผ่าน</p>
    ...
    @push('scripts')
        ...
        <script>
            function studentDataImportManager() {
                return {
                    years: [],
                    imports: [],
                    dataSets: [],
                    ...
                    form: {
                        ...
                        term: '1',
                        mode: 'replace',
                        data_type: @json($defaultDataType),
                        file: null,
                        fileName: ''
                    },
                    preview: {
                    ...
                    fetchData() {
                        axios.get('{{ route('admin.student-data-imports.data') }}')
                            .then(response => {
                                if (response.data.status === 'success') {
                                    this.years = response.data.years || [];
                                    this.imports = response.data.imports || [];
                                    this.dataSets = response.data.data_sets || [];
                                    this.dataTypes = response.data.data_types || this.dataTypes;
                                    ...
                                    if (!this.dataTypes[this.form.data_type]) {
                                        this.form.data_type = Object.keys(this.dataTypes)[0] || '';
                                    }
                                }
                            })
                            .catch(() => this.showToast('ไม่สามารถโหลดข้อมูลนำเข้าได้', 'error'));
                    },
                    ...
                    previewFile() {
                        ...
                        if (!this.form.data_type) {
                            this.showToast('กรุณาเลือกชนิดข้อมูล', 'error');
                            return;
                        }
                        if (!this.form.file) {
                        ...
                        formData.append('data_type', this.form.data_type);
                        formData.append('file', this.form.file);

                        axios.post('{{ route('admin.student-data-imports.preview') }}', formData, {
                            ...
                            this.preview = {
                                uploadToken: '',
                                sourceFilename: '',
                                summary: error.response?.data?.preview || null
                            };
                            this.showToast(error.response?.data?.message || 'ไม่สามารถตรวจสอบไฟล์ได้', 'error');
                        }).finally(() => {
                    ...
                    summaryCards() {
                        ...
                        return [
                            { label: 'แถวทั้งหมด', value: this.preview.summary.total_rows || 0 },
                            { label: 'แถวที่อ่านได้', value: this.preview.summary.valid_rows || 0 },
                            { label: 'แถวไม่สมบูรณ์', value: this.preview.summary.invalid_rows || 0 },
                            { label: 'ไม่ match โรงเรียน', value: this.preview.summary.unmatched_rows || 0 },
                        ];
                    },
                    ...
                    selectedDataSetSummary() {
                        return this.dataSets.find(item =>
                            ...
                            String(item.data_type) === String(this.form.data_type)
                        ) || null;
                    },

                    selectedTypeLabel() {
                        return this.dataTypes[this.form.data_type]?.label || this.form.data_type || '-';
                    },
