...
<x-layout>
    <x-slot:title>เปลี่ยนรหัสผ่าน | {{ $webName }}</x-slot>
    ...
    <main class="py-16 md:py-24 bg-slate-50/50 min-h-screen">
        ...
        <section class="max-w-7xl mx-auto px-6 text-center space-y-4 mb-16">
            ...
            <p class="text-slate-500 text-sm max-w-2xl mx-auto leading-relaxed">
                เปลี่ยนรหัสผ่านเพื่อรักษาความปลอดภัยและความปลอดภัยของบัญชีของคุณ
            </p>
        </section>
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex items-center gap-4 pt-4 border-t border-slate-100">
                        <button type="button"
                            ...
                            <span x-text="saving ? 'กำลังบันทึก...' : 'บันทึกรหัสผ่านใหม่'"></span>
                        </button>
                        <a href="{{ route('profile.edit') }}"
                           class="text-slate-500 hover:text-slate-700 font-bold text-xs py-3.5 px-6 rounded-2xl border border-slate-200 hover:border-slate-300 transition">
                            กลับไปที่โปรไฟล์
                        </a>
                    </div>
                </div>
    ...
    window.addEventListener('load', function() {
        // Alpine data is defined inline in x-data, but we wrap custom functions inside standard load listeners to follow security policies.
    });
